...
export default class TryItOutButton extends React.Component {
  ...
  render() {
    const { onTryoutClick, onCancelClick, showHistory, onClickHistory, onResetClick, enabled, hasUserEditedBody, isOAS3 } = this.props
    ...
    return (
      <div className={showReset ? "try-out btn-group" : "try-out"}>
        {
          enabled ? <>
            { showHistory && <button className="btn btn-history" onClick={ onClickHistory }>History</button> }
